...
public class Generator {
    ...
    public static String generateRandomEmail(Integer length) {
        List<String> emails = new ArrayList<>(Arrays.asList("mail.ru", "yandex.ru", "gmail.com"));
        StringBuilder resultStr = new StringBuilder();
        for (int i = 0; i < length; i++)
            resultStr.append(englishAlphabet.charAt(random.nextInt(englishAlphabet.length())));
        return resultStr.toString() + "@" + emails.get(random.nextInt(emails.size()));
    }
}
